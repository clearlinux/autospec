...
failed_pats = [
    ...
    (r"ERROR:  Could not find a valid gem '([a-zA-Z0-9\-\_]*)' \([>=]+ ([0-9.]+).*\) " r"in any repository", 0, 'ruby'),
    ...
    (r"\> Could not find ([^:\s]+):([^:\s]+):([^:\s]+)\.$", 0, 'maven'),
    (r"\[ERROR\] .* Cannot access \S+ \(.*\) in offline mode and the artifact " r"(.+:.+:(?:pom|jar|signature)(?::.+)?):(.*) has not been downloaded from it before.*", 0, 'maven'),
    (r"\[WARNING\] The POM for (.+:.+:(?:jar|pom)(?::.+)?):(.*) is missing, no dependency information " r"available", 0, 'maven'),
